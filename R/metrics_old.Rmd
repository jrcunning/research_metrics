...
new_metrics_file <- list.files(path = "data", pattern = "new 1", full.names = TRUE)
...
metrics <- metrics %>%
  mutate(Category = case_when(grepl("Media", Category) ~ "Media opportunity",
                               ...
                               grepl("takeholder", Category) ~ "Capacity Sharing & Training",
                               grepl("Community", Category) ~ "Capacity Sharing & Training",
                               grepl("Capacity", Category) ~ "Capacity Sharing & Training",
                               grepl("Publication", Category) ~ "Publication",
                               grepl("active meeting", Category) ~ "Presentation",
                               TRUE ~ Category))
```
...
# Dedupe combined references
library(stringdist)
hc_references <- hc_references %>%
  { g <- cutree(
      hclust(
        as.dist(stringdistmatrix(.$reference, method = "jw")),
        method = "average"
      ),
      h = 0.10
    )
...
# Publications
```{r}
# Filter out duplicated publications
library(stringdist)
nondupes <- metrics %>%
  filter(Category == "Publication") %>%
  { g <- cutree(hclust(as.dist(stringdistmatrix(.$Citation, method = "jw")), "average"), h = 0.10)
    .[!duplicated(g), ] } 
...
# dupes <- metrics %>%
#   filter(Category == "Publication") %>%
#   { g <- cutree(hclust(as.dist(stringdistmatrix(.$Citation, method = "jw")), 
#                        method = "average"), h = 0.10)
#     .[duplicated(g) | duplicated(g, fromLast = TRUE), ] }
...
pubsumm %>% 
  ...
  theme_custom() +
  theme(legend.position = "none") +
  scale_x_continuous(breaks = function(x) round(seq(min(x), max(x), by = 1), 0)) +
  labs(x = "", y = "", title = "Number of Scientific Publications")
```
...
# Plot Citations annually
citesumm %>%
  group_by(Year) %>%
  summarize(Citations = sum(Citations)) %>%
  ggplot(aes(x = Year, y = Citations, fill = factor(Year))) +
  geom_bar(stat = "identity", alpha = 0.7) +
  geom_text(aes(label = ifelse(Citations == 0, "", Citations)), color = "white", nudge_y = -20) +
  theme_custom() +
  theme(legend.position = "none") +
  scale_x_continuous(breaks = function(x) round(seq(min(x), max(x), by = 1), 0)) +
  labs(x = "", y = "", title = "Number of Citations")
...
```{r}
# New title: Capacity Sharing & Training
# Old title: Community engagement -- unique individuals

engagement.unique.ind <- metrics %>%
  filter(Category == "Capacity Sharing & Training") %>%
  group_by(Year, Quarter) %>%
  summarize(Quantity = sum(Quantity, na.rm = TRUE)) %>%
  bind_rows(filter(pre2019, Category == "Community engagement")) %>%
  select(Year, Quarter, Quantity) %>%
  ungroup() %>%
  complete(Year, Quarter, fill = list(Quantity = 0))


# Some numbers are super high
# metrics %>%
#   filter(Category == "Community engagement") %>%
#   filter(Quantity > 1000)
# 
# engagement.unique.ind %>%
#   arrange(-Quantity)

# Plot community engagement quarterly
ggplot(engagement.unique.ind, aes(x = Quarter, y = Quantity, fill = factor(Year))) +
  ...
  theme(legend.position = "none",
        axis.text.x = element_text(size = 7)) +
  labs(x = "", y = "", title = "Number of people engaged")

# Plot community engagement annually
engagement.unique.ind %>%
  group_by(Year) %>%
  summarize(Quantity = sum(Quantity)) %>%
  ggplot(aes(x = Year, y = Quantity, fill = factor(Year))) +
